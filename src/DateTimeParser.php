...
/**
 * DateTimeParser.
 * Taken from https://github.com/sabre-io/vobject/blob/master/lib/DateTimeParser.php
 *
...
class DateTimeParser
{
    ...
    public static function parseDuration($duration, $asString = false)
    {
        ...
        if (!$result) {
            throw new ICalValidationException('The supplied iCalendar duration value is incorrect: '.$duration);
        }
    ...
    public static function parseVCardDateTime($date)
    {
        ...
        if (!preg_match($regex, $date, $matches)) {
            ...
            if (!preg_match($regex, $date, $matches)) {
                throw new ICalValidationException('Invalid vCard date-time string: '.$date);
            }
        }
        $parts = [
            'year',
    ...
    public static function parseVCardTime($date)
    {
        ...
        if (!preg_match($regex, $date, $matches)) {
            ...
            if (!preg_match($regex, $date, $matches)) {
                throw new ICalValidationException('Invalid vCard time string: '.$date);
            }
        }
        $parts = [
            'hour',
